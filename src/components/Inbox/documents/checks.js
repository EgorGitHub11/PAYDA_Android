...
export default class checks extends Component {
  ...
  getData(){
    const {jwt,pk,docUrl} = this.state
    const url = mainUrl+`/api/archive/${pk}/`
      ...
      .then((response) => {
        ...
        this.setState({arr: response.data.checks})
        this.setState({docUrl: response.data.checks[0].file})
        // this.setState({docFormat:newUrl})
        // console.log(this.state.docFormat)
  ...
  download(){
    const {docUrl,jwt,docFormat} = this.state
  ...
  render() {
    ...
    return (
      ...
      <Header navigation={this.props.navigation} name={'ЧЕКИ'}/>
      <View style={childMainContainer}>
      <ScrollView style={scroll}>
          {
                  arr.map(i => (
                    i.length == 0 ? <Text>Empty</Text> : 
